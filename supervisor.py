"""
BTC 收益增强策略 - 进程守护 (supervisor)

职责：
- 每 CHECK_INTERVAL 秒检查 5050 端口是否存活
- 端口死：pythonw 重拉 app.py（进程崩溃自愈）
- 端口活但实例 status != running：自动 POST /btc-enhancer/api/start 触发策略
...
CHECK_INTERVAL = 10
START_GRACE = 20
STATUS_URL = f"http://{HOST}:{PORT}/btc-enhancer/api/status"
START_URL = f"http://{HOST}:{PORT}/btc-enhancer/api/start"
...
def launch():
    logging.info("重拉 BTC 实例: pythonw app.py")
    try:
        subprocess.Popen(
            [PYTHONW, "app.py"],
            cwd=WORK_DIR,
...
def main():
    _setup_logging()
    logging.info("=== BTC supervisor 启动, 监控端口 %d ===", PORT)
    first = True
    while True:
        try:
            if not port_alive():
